...
import { createStorageProxy, storage } from "@/storage";

describe("createStorageProxy()", () => {
  let storage: ReturnType<typeof createStorageProxy>;
  beforeEach(() => {
    localStorage.clear();
    storage = createStorageProxy("_spy_dummy_");
  });
  ...
  describe("delete()", () => {
    ...
    it("succeeds if matched key existed", () => {
      ...
      localStorage.setItem("_spy_dummy_dummy", "dummy");
  ...
  describe("keys", () => {
    ...
    it("prefix is not added", () => {
      localStorage.setItem("_spy_dummy_dummy", "dummyData");
  ...
  describe("setting property", () => {
    it("can set value", () => {
      ...
      expect(storage.keys).toHaveLength(1);
      expect(localStorage.getItem("_spy_dummy_dummy")).toBe("dummyDataUpdated");
